...
import { authHeaders, clearUserAmenities, getAmenityTypes, getUserPreferences, getUserRequiredAmenities } from '../api.js';
...
export default function Preferences() {
    ...
    useEffect(() => {
        Promise.all([
            getUserPreferences(),
            getUserRequiredAmenities(),
        ]).then(([prefs, amenities]) => {
            if (prefs && prefs.length > 0) {
                const latest = prefs[prefs.length - 1];
                setNoiseLevel(latest.noise_level || 5);
                setCrowdLevel(latest.crowd_level || '');
            }
            if (amenities && amenities.length > 0) {
                const amenityMap = {};
                amenities.forEach(a => {
                    amenityMap[a.amenity_type_id] = a.priority || 1;
                });
