...
//initializing leaflet map from notes
    ...
    async function handleCheckIn(loc) {
        try {
            let locationId = (loc.location_id) ? loc.location_id : loc.locationId;
            ...
            if (checkedInFromRecRef.current && loc) {
                //const userId = localStorage.getItem('user_id');

                let result = await postRecommendation(
                    locationId,
                    loc.matchCount/loc.totalPreferences,
                    loc.preferences.find(p => p.preference.includes('noise'))?.detail.split(": ")[1].split("/")[0] || '',
                    loc.preferences.find(p => p.preference.includes('crowd'))?.detail.split(": ")[1] || ''
                );
                setFollowupRecId(result.recommendation.recommendation_id);
            }
        } catch {
    ...
    async function handleFeedbackSubmit() {
        ...
        const payload = {
            recommendation_id: followupRecId,
...
// Recommendation modal
    ...
    const[followupRecId, setFollowupRecId] = useState(false);
    //const[selectRecLoc, setSelectRecLoc] = useState(null);
    ...
    async function handleGetRecommendation() {
        ...
        try {
            const data = await fetchWithRefresh(`${BASE_URL}/api/recommendations/generate`, {
                method: 'GET',
                headers: authHeaders()
            });
    ...
    return (
        <div className={"container py-4 amenitiesText" }>
            <div className="mb-4 locationsText">
                ...
                <p className='text-dark amenitiesText '> Browse available locations and their current status </p>
            </div>
            {checkInMessage && (
            ...
            <div className="d-flex gap-3 mb-4 ">
                ...
                <div className="card preferencesCard flex-grow-1">
                    <div className="card-body">
                        ...
                        <p className="card-text amenitiesText">Find your best location based on your preferences</p>
                    ...
                    <div className="card border-0 shadow-lg report-modal-card"
                        ...
                        <div className="card-body p-4">
                            ...
                            <p className="report-modal-subtitle mb-4 small">
                                Based on your saved preferences here are your best locations.
                            </p>
                                            ...
                                            {loc.matchScore} match
                                               ...
                                               {p.met ? '✅' : '❌'} {p.preference.includes('noise_level')
                                                ...
                                                <button
                                                    ...
                                                    style={{backgroundColor: '#6b8f71', color: 'white'}}
                                                    onClick={async () => {
                                                        // setstatevariablehere(i)
                                                        checkedInFromRecRef.current = true;
                                                        await handleCheckIn(loc);
                                                    }}
