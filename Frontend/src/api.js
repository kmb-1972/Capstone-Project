...
export const postRecommendation = async ( location_id, recommendation_score, location_noise, location_crowd) =>{
    const response = await fetch(`${BASE_URL}/api/recommendations`, {
        method: "POST",
        headers:authHeaders(),
        body: JSON.stringify({ location_id, recommendation_score, location_noise, location_crowd})
    });
    return response.json();
};